...
- **Schema migrations**: idempotent ALTER TABLEs and `DO $$ ... IF NOT EXISTS (SELECT 1 FROM schema_version WHERE version = N) ... INSERT INTO schema_version VALUES (N) ... END $$`. To add a migration, append a new block, then bump tracked version. **Current version is 13** (adds `posts` + `post_notes` tables for the Posts feature).
